...
(function () {
  ...
  var DICT = {
    ...
    "zh-Hans": {
      ...
      "db.hero.title.em": "题目",
      "db.hero.title.post": "",
      "db.hero.lede": "大多数检索增强生成的演示只回答一个问题：知识库里有答案的时候，系统答得好不好。那是简单的情况。危险的失败发生在没有答案的时候，用户问了文件从未涵盖的东西，模型得在“我不知道”和编一个听起来合理的说法之间做选择。",
      "db.hero.lede2": "这个项目为一个虚构的 SaaS 产品搭了一套小型 RAG 系统，再用一组专门设计的探测题和一个独立的跨模型家族评判者，针对那个失败模式做压力测试，验证这套系统有没有如实交代自己知道什么。",

      "db.spec.role": "角色",
      "db.spec.role.value": "评估方法设计与实现",
      "db.spec.kb": "知识库",
      ...
      "db.notice.body": "这是一个独立的技术展示。Driftboard 是为此虚构出来的项目管理工具，并非真实公司，本页描述的任何内容都不涉及任何真实产品、客户或正式上线的系统。",

      "db.setup.eyebrow": "测试对象",
      "db.setup.title": "究竟测的是什么。",
      "db.setup.body1": "知识库是九份 markdown 文件，涵盖一个虚构 SaaS 产品的定价、权限、集成、API 限额、数据政策和计费规则，合计约十五页。每份文件按标题切块，用 Voyage AI 产生向量，再依查询以余弦相似度检索。这个规模不需要向量数据库。Claude Haiku 4.5 只根据检索回来的内容撰写答案。",
      "db.setup.body2": "RAG 流程本身是标准做法。这个项目真正的重点，是那组刻意要让它出错的探测题库，以及一层不盲目相信自己的评估机制。",
      ...
      "db.probe3.name": "真正的空白",
      "db.probe3.body": "知识库完全没有触及这个主题。这时候唯一正确的回答就是说自己不知道。",
      "db.probe4.name": "改写配对",
      "db.probe4.body": "同一个问题用两种说法各问一次，检查系统前后是否自相一致。",
      "db.probes.note": "软性无解和真正的空白表面上很像，两者都会得到“否”或“未涵盖”的回应，但测的是不同的东西。把两者混为一谈，是这个项目自身的评估设计里最大的噪声来源，下面会谈到。",

      "db.judge.eyebrow": "评判者",
      "db.judge.title": "要抓出幻觉，得先知道模型当时实际看到了什么。",
      "db.judge.body": "如果评判者是拿产品的一般知识来评一个答案，那它测的其实是这个答案碰巧对不对，跟这个答案有没有依据是两回事。这里的评判者只看得到生成模型在那一次调用看到的东西：问题、检索到的片段，以及答案本身。没有标准答案、没有类别、没有提示。它必须从六种结果里选一个；这个选择由 JSON schema 强制约束，不合法的标签在结构上就不可能出现。",

      "db.labels.caption": "评判标签",
      "db.labels.h1": "标签",
      "db.labels.h2": "含义",
      "db.label1.name": "有依据且正确",
      "db.label1.body": "答案里的每一项陈述都有检索片段支持，而且与之相符。",
      "db.label2.name": "有依据但错误",
      ...
      "db.label6.name": "拒答夹带捏造",
      "db.label6.body": "主体是一次正确的拒答，却仍然附带了一句没有依据的具体说法。",
      "db.judge.validation": "评判者跑在与生成模型不同的模型家族上，避免让模型改自己的考卷。它以人工标注的黄金题组做验证，用的是 Cohen's kappa 而不是原始准确率，因为 kappa 会扣掉标签本身猜中的概率，准确率不会。",

      "db.found.eyebrow": "结果",
      "db.found.title": "有意思的从来不是那个摆在标题上的数字。",
      "db.found.retrieval": "<strong>在所有已知正确来源的探测题上，检索有 97.2% 的概率能把正确文件排进前四名。</strong>",
      "db.retrieval.caption": "检索准确率",
      "db.retrieval.h1": "指标",
      "db.retrieval.h2": "结果",
      "db.retrieval.r1": "Hit@1（正确来源排在第一）",
      "db.retrieval.r2": "Hit@4（正确来源在前四名内）",
      "db.found.miss": "这是个漂亮的数字，很容易就写进简报然后翻页。但更有用的发现是完全没命中的那 2.8%。其中一题问的是两个工作区能不能合并，检索回来的四个片段谈的是集成、角色和权限，没有一个是真正写着答案的 FAQ 条目。模型给的回应仍然准确，也和真实政策高度吻合，几乎可以确定是因为“工作区合并通常不支持，请联系客服”这个模式够常见，Claude Haiku 是凭一般知识生出来的，而不是根据系统交给它的内容。只看输出，它像是一个干净、有依据的答案。只有把检索记录和标准答案交叉比对，才会暴露出这个回应没有任何一句是那次检索内容支持的。这才是最要命的失败：答案对了，但理由是错的，而且不主动去查就看不见。",
      "db.found.judge": "<strong>评判者的验证差点骗了我们，而修正这件事本身，才是这个项目真正的重点。</strong>第一轮验证跑出了一个真实的分歧：有六题人工标注为正确拒答，评判者却判成有依据且正确。这六题全部是软性无解，没有一题是真正的空白。这个分布暴露的是标签体系本身的缺陷：知识库既然明确写出了一个否定事实，那就该算成有依据的答案，而不是拒答。于是修正了定义，并重新标注受影响的题目。",
      "db.found.circular": "陷阱就在这里。修正之后重跑，结果是完全一致，每个类别的 kappa 都是 1.000。这个数字是循环论证的症状，不是评判者准确的证据：标准答案刚刚往评判者自己的读法靠过去，这个一致是设计出来的，不是检验出来的。",
      "db.found.holdout": "解法是第二组互不重叠的探测题：十四道针对同一个类别边界的新题目，独立标注，且在标注时完全不知道评判者后来会怎么判。这样才算重新建立起一个真正独立的检验，诚实的数字也是从这里来的。",
      ...
      "db.heldout.r3": "全部保留题",
      "db.found.disagreements": "完整报告里记录的不是零分歧，而是两个，各自附上判断理由。拿一份已经对齐过的标准答案跑出满分，放在投影片上比较好看；一个带着可见分歧的保留题分数，才是有意义的那个。",

      "db.why.eyebrow": "为什么这才是重点",
      "db.why.title": "这是差异所在，不是附注。",
      "db.why.body1": "上面写的大半，是一个自己抓自己错误的过程：一套把两个不同问题混在一起的标签体系、一个绕回自己身上的验证流程，以及一个真的去查之后才发现根本没有依据的“正确”答案。这些没有一个是靠读最终输出看出来的。它们来自刻意拿检索记录去对标注、再拿标注去对一组从未动过的保留题，然后才决定某个数字值不值得报出去。",
      "db.why.body2": "这就是这类评估工作在真实上线系统上该被要求的标准：这个数字，撑不撑得住别人刻意来挑洞。",

      "db.stack.eyebrow": "技术组成",
      "db.stack.value": "Python · Claude Haiku 4.5（生成）· GPT-5.6 Terra（跨家族评判）· Voyage AI 向量模型 voyage-3.5-lite · OpenRouter · 余弦相似度检索，不用向量数据库 · Cohen's kappa 验证，自行实现",
      "db.link.repo": "GitHub 仓库",
    ...
    "zh-Hant": {
      ...
      "db.hero.title.em": "題目",
      "db.hero.title.post": "",
      "db.hero.lede": "大多數檢索增強生成的示範只回答一個問題：知識庫裡有答案的時候，系統答得好不好。那是簡單的情況。危險的失敗發生在沒有答案的時候，使用者問了文件從未涵蓋的東西，模型得在「我不知道」和編一個聽起來合理的說法之間做選擇。",
      "db.hero.lede2": "這個專案為一個虛構的 SaaS 產品搭了一套小型 RAG 系統，再用一組專門設計的探測題和一個獨立的跨模型家族評判者，針對那個失敗模式做壓力測試，驗證這套系統有沒有如實交代自己知道什麼。",
      ...
      "db.notice.body": "這是一個獨立的技術展示。Driftboard 是為此虛構出來的專案管理工具，並非真實公司，本頁描述的任何內容都不涉及任何真實產品、客戶或正式上線的系統。",

      "db.setup.eyebrow": "測試對象",
      "db.setup.title": "究竟測的是什麼。",
      "db.setup.body1": "知識庫是九份 markdown 文件，涵蓋一個虛構 SaaS 產品的定價、權限、整合、API 限額、資料政策和計費規則，合計約十五頁。每份文件按標題切塊，用 Voyage AI 產生向量，再依查詢以餘弦相似度檢索。這個規模不需要向量資料庫。Claude Haiku 4.5 只根據檢索回來的內容撰寫答案。",
      ...
      "db.probes.note": "軟性無解和真正的空白表面上很像，兩者都會得到「否」或「未涵蓋」的回應，但測的是不同的東西。把兩者混為一談，是這個專案自身的評估設計裡最大的雜訊來源，下面會談到。",

      "db.judge.eyebrow": "評判者",
      "db.judge.title": "要抓出幻覺，得先知道模型當時實際看到了什麼。",
      "db.judge.body": "如果評判者是拿產品的一般知識來評一個答案，那它測的其實是這個答案碰巧對不對，跟這個答案有沒有依據是兩回事。這裡的評判者只看得到生成模型在那一次呼叫看到的東西：問題、檢索到的片段，以及答案本身。沒有標準答案、沒有類別、沒有提示。它必須從六種結果裡選一個；這個選擇由 JSON schema 強制約束，不合法的標籤在結構上就不可能出現。",

      "db.labels.caption": "評判標籤",
      "db.labels.h1": "標籤",
      "db.labels.h2": "含義",
      "db.label1.name": "有依據且正確",
      "db.label1.body": "答案裡的每一項陳述都有檢索片段支持，而且與之相符。",
      "db.label2.name": "有依據但錯誤",
      ...
      "db.label6.name": "拒答夾帶捏造",
      "db.label6.body": "主體是一次正確的拒答，卻仍然附帶了一句沒有依據的具體說法。",
      "db.judge.validation": "評判者跑在與生成模型不同的模型家族上，避免讓模型改自己的考卷。它以人工標註的黃金題組做驗證，用的是 Cohen's kappa 而不是原始準確率，因為 kappa 會扣掉標籤本身猜中的機率，準確率不會。",

      "db.found.eyebrow": "結果",
      "db.found.title": "有意思的從來不是那個擺在標題上的數字。",
      ...
      "db.found.miss": "這是個漂亮的數字，很容易就寫進簡報然後翻頁。但更有用的發現是完全沒命中的那 2.8%。其中一題問的是兩個工作區能不能合併，檢索回來的四個片段談的是整合、角色和權限，沒有一個是真正寫著答案的 FAQ 條目。模型給的回應仍然準確，也和真實政策高度吻合，幾乎可以確定是因為「工作區合併通常不支援，請聯絡客服」這個模式夠常見，Claude Haiku 是憑一般知識生出來的，而不是根據系統交給它的內容。只看輸出，它像是一個乾淨、有依據的答案。只有把檢索紀錄和標準答案交叉比對，才會暴露出這個回應沒有任何一句是那次檢索內容支持的。這才是最要命的失敗：答案對了，但理由是錯的，而且不主動去查就看不見。",
      "db.found.judge": "<strong>評判者的驗證差點騙了我們，而修正這件事本身，才是這個專案真正的重點。</strong>第一輪驗證跑出了一個真實的分歧：有六題人工標註為正確拒答，評判者卻判成有依據且正確。這六題全部是軟性無解，沒有一題是真正的空白。這個分布暴露的是標籤體系本身的缺陷：知識庫既然明確寫出了一個否定事實，那就該算成有依據的答案，而不是拒答。於是修正了定義，並重新標註受影響的題目。",
      "db.found.circular": "陷阱就在這裡。修正之後重跑，結果是完全一致，每個類別的 kappa 都是 1.000。這個數字是循環論證的症狀，不是評判者準確的證據：標準答案剛剛往評判者自己的讀法靠過去，這個一致是設計出來的，不是檢驗出來的。",
      "db.found.holdout": "解法是第二組互不重疊的探測題：十四道針對同一個類別邊界的新題目，獨立標註，且在標註時完全不知道評判者後來會怎麼判。這樣才算重新建立起一個真正獨立的檢驗，誠實的數字也是從這裡來的。",
      ...
      "db.heldout.r3": "全部保留題",
      "db.found.disagreements": "完整報告裡記錄的不是零分歧，而是兩個，各自附上判斷理由。拿一份已經對齊過的標準答案跑出滿分，放在投影片上比較好看；一個帶著可見分歧的保留題分數，才是有意義的那個。",

      "db.why.eyebrow": "為什麼這才是重點",
      "db.why.title": "這是差異所在，不是附註。",
      "db.why.body1": "上面寫的大半，是一個自己抓自己錯誤的過程：一套把兩個不同問題混在一起的標籤體系、一個繞回自己身上的驗證流程，以及一個真的去查之後才發現根本沒有依據的「正確」答案。這些沒有一個是靠讀最終輸出看出來的。它們來自刻意拿檢索紀錄去對標註、再拿標註去對一組從未動過的保留題，然後才決定某個數字值不值得報出去。",
