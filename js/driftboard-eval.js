...
(function () {
  ...
  var DICT = {
    ...
    "zh-Hans": {
      ...
      "db.hero.title.em": "题目",
      "db.hero.title.post": "。",
      "db.hero.lede": "大多数检索增强生成的演示只回答一个问题：知识库里有答案的时候，系统答得好不好。那是容易的一半。危险的失败发生在没有答案的时候，用户问了文件从未涵盖的东西，模型得在“我不知道”和编一个听起来合理的说法之间做选择。",
      "db.hero.lede2": "这个项目为一个虚构的 SaaS 产品搭了一套小型 RAG 系统，再用一组专门设计的探测题和一个独立的跨模型家族评判者，针对那个失败模式做压力测试，验证系统对自己知道什么这件事有没有说实话。",
      ...
      "db.notice.body": "这是一个独立的技术展示。Driftboard 是为这个项目虚构出来的项目管理工具，并非真实公司，本页描述的任何内容都不涉及任何真实产品、客户或正式上线的系统。",

      "db.setup.eyebrow": "测试对象",
      "db.setup.title": "实际被测的是什么。",
      "db.setup.body1": "知识库是九份 markdown 文件，涵盖一个虚构 SaaS 产品的定价、权限、集成、API 限额、数据政策和计费规则，合计约十五页。每份文件按标题切块，用 Voyage AI 产生向量，再以余弦相似度对查询检索。这个规模不需要向量数据库。Claude Haiku 4.5 只根据检索回来的内容撰写答案。",
      ...
      "db.probes.note": "软性无解和真正的空白表面上很像，两者都会得到“否”或“未涵盖”的回应，但测的是不同的东西。把两者混为一谈，结果成了这个项目自身评估设计里最大的一个噪声来源，下面会谈到。",

      "db.judge.eyebrow": "评判者",
      "db.judge.title": "要抓出幻觉，得先知道模型当时实际看到了什么。",
      "db.judge.body": "如果评判者是拿产品的一般知识来评一个答案，那它测的其实是这个答案碰巧对不对，跟这个答案有没有依据是两回事。这里的评判者只看得到生成模型在那一次调用看到的东西：问题、检索到的片段，以及答案本身。没有标准答案、没有类别、没有提示。它必须从六种结果里选一个，并由 JSON schema 强制约束，让不合法的标签在结构上不可能出现。",
      ...
      "db.found.miss": "这是个漂亮的数字，很容易就写进简报然后翻页。但更有用的发现是完全没命中的那 2.8%。其中一题问的是两个工作区能不能合并，检索回来的四个片段谈的是集成、角色和权限，没有一个是真正写着答案的 FAQ 条目。模型给的回应仍然准确，也和真实政策高度吻合，几乎可以确定是因为“工作区合并通常不支持，请联系客服”这个模式够常见，Claude Haiku 是凭一般知识生出来的，而不是凭手上拿到的任何东西。只看输出，它像是一个干净、有依据的答案。只有把检索记录和标准答案交叉比对，才会暴露出这个回应没有任何一句是那次检索内容支持的。这才是最要命的失败：答案对了，但理由是错的，而且不去翻就看不见。",
      "db.found.judge": "<strong>评判者的验证差点骗了我们，而修正这件事本身，才是这个项目真正的重点。</strong>第一轮验证跑出了一个真实的分歧：有六题人工标注为正确拒答，评判者却判成有依据且正确。这六题全部是软性无解，没有一题是真正的空白。这个分布暴露的是标签体系本身的缺陷：知识库既然明确写出了一个否定事实，那就该算成有依据的答案，而不是拒答。于是定义被修掉，受影响的题目重新标注。",
      ...
      "db.why.body1": "上面写的大半，是一个抓自己错误的故事：一套把两个不同问题混在一起的标签体系、一个绕回自己身上的验证流程，以及一个真的去查之后才发现根本没有依据的“正确”答案。这些没有一个是靠读最终输出看出来的。它们来自刻意拿检索记录去对标注、再拿标注去对一组从未碰过的保留题，然后才决定某个数字值不值得报出去。",
    ...
    "zh-Hant": {
      ...
      "db.why.body1": "上面寫的大半，是一個抓自己錯誤的故事：一套把兩個不同問題混在一起的標籤體系、一個繞回自己身上的驗證流程，以及一個真的去查之後才發現根本沒有依據的「正確」答案。這些沒有一個是靠讀最終輸出看出來的。它們來自刻意拿檢索紀錄去對標註、再拿標註去對一組從未碰過的保留題，然後才決定某個數字值不值得報出去。",
