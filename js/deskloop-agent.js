...
(function () {
  ...
  var DICT = {
    ...
    "zh-Hans": {
      "dl.meta.title": "DeskLoop：分级授权的 IT 与 HR 助手 | Miloop AI",
      "dl.meta.description": "一套以分级工具服务器为核心的 IT 与 HR 助手：哪些动作 Agent 可以自己做，哪些必须先拿到人的同意，哪些根本没有执行路径。",
      ...
      "dl.spec.role": "承担角色",
      "dl.spec.role.value": "Agent 架构、权限设计、评估",
      "dl.spec.boundary": "边界",
      "dl.spec.boundary.value": "独立进程中的工具服务器，通过 stdio 通信",
      "dl.spec.evidence": "证据",
      "dl.spec.evidence.value": "12道保留题，只跑一次，零严重失效",

      "dl.notice.label": "虚构情境",
      "dl.notice.body": "这是一个独立的技术演示。DeskLoop 所服务的雇主 Quillstone 是为这个项目虚构出来的，里面的每一位员工、每一张工单、每一笔申请和每一份政策文件也都是。这里展示的东西没有、也从未投入生产使用，本页也没有任何内容描述真实的客户或真实的系统。",

      "dl.mech.eyebrow": "机制",
      "dl.mech.title": "三个层级，其中一个根本没有代码路径。",
      "dl.mech.body1": "一套 Agent 系统真正值得看的地方，不是它的工具清单，而是从 Agent 决定要动手，到动作真的发生，这中间发生了什么。DeskLoop 里的每个工具都注册在三个层级之一，而这份注册表位于进程边界的服务器那一侧，被攻陷的提示词伸不进去。",
      ...
      "dl.mech.body2": "所以 Tier 1 的工具永远不会只被调用一次。第一次调用返回的是 <code>CONFIRMATION_REQUIRED</code>，附带一份规范化的动作描述，而不是执行它。Agent 把这份描述渲染成一个提案，状态图在这里停下。用户回答之后，由<strong>另一次独立的</strong>模型调用在受限输出下把这个回答分类成确认、修改或取消。只有“确认”才会指示服务器签发令牌。",
      ...
      "dl.clips.body1": "对话记录会把这件事藏起来。如果 Agent 说它已经提交了你的请假申请，读的人无从判断关卡到底被执行了，还是模型只是这么说而已。所以这个演示界面是聊天区旁边一块实时的仪表面板：每一次工具调用连同它的层级标记、每一条拒绝理由、每一次确认分类，以及底层数据库的状态，是变了还是没变，都在你眼前。",
      "dl.clips.body2": "这里没有在线试用版本，这是刻意的。一个公开的聊天端点，每位访客的每一轮都在烧真实额度；而一个钉死在某个模型 id 上的线上演示会悄悄腐坏，然后在最糟糕的时刻失灵。取而代之的展示面就是这几段录屏，而里面的每一件事都可以不用任何密钥自行核验：整套测试无需凭证即可运行，下面所有数字都能由一个脚本从已提交的产物中重算出来。",
      "dl.clips.note": "未经剪辑的屏幕录制。无声音，无剪接。",

      "dl.clip1.name": "令牌绑定",
      "dl.clip1.body": "用户先申请三天假，然后一边回答“是的，我确认”，一边在同一句话里把申请改成五天。这个确认是真的，只是它不属于这个动作：服务器的令牌绑定在签发时的那组精确参数上，所以假期余额始终停在 3.5。Agent 先说权限问题，再说余额问题，顺序就是这样。",
      "dl.clip2.name": "正常流程长什么样",
      "dl.clip2.body": "一个动作，在审计面板上是两行：先黄后绿。黄色那行是工具服务器拒绝执行，并把它本来会做的事描述了回来。两行之间隔着的，只有用户的一个回答。顺带一提，它把严重程度定为 3，依据是用户提到还有替代办法可用，不是因为语气听起来有多急。",
      "dl.clip3.name": "试着弄坏它",
      "dl.clip3.body": "三次绕过尝试：借来的权威、伪造的系统指令，以及声称某个 Tier 2 工具其实只是无害的读取。前两次落在和普通请求一模一样的黄色行上，因为服务器根本没有去读那段说辞。第三次转交给人处理。这里没有任何令牌能让重置密码执行，代码里也没有那条路径。这不是检测出来的，因为压根没有需要检测的东西。",
      "dl.clip4.name": "步骤是它自己排出来的",
      "dl.clip4.body": "一句指令，没有一步是用户指名的：读申请历史、拿驳回理由去对员工记录、然后提出重新提交并停下来等确认。是两次工具调用而不是三次，因为 <code>get_request_history</code> 已经带回了在职天数和试用期门槛，不需要再单独查一次政策。这一条里 Agent 还先说出了当初被驳回的理由，才提出重新申请。它并不总是这么做：评估里有五道题，回复是直接跳到提案的，报告如实写了。",
      "dl.clip5.name": "那些数字",
      ...
      "dl.rec.body1": "那排绕过按钮就是拿来按的，所以第03支的第一次拍摄连按了三颗。脚本预期面板上会出现三行，实际拍到的是一行，外加两次结果为 unclear 的确认检查。",
      "dl.rec.body2": "一个更早做出的决定精确地解释了这件事：只要有提案还悬着，Agent 就不绑定任何工具。第一次尝试留下了一个未决的提案，于是第二颗按钮送出的文字根本不是一个新请求，它是用户对那个提案的回答；而分类器不会把一段伪造的系统指令读成对请假申请的同意。它返回 unclear，提案继续悬着，第三颗按钮撞上同一堵墙。这里的每一步都是系统在按设计运作，包括一个只会往 unclear 失败、绝不会往 confirm 失败的分类器。",
      "dl.rec.clip.index": "废弃的一条",
      "dl.rec.clip.name": "按了三次，只出一行",
      "dl.rec.clip.body": "这是被丢掉的那一条，留下来是因为它拍到的东西。三次尝试在聊天里都被正确回应了，而面板几乎什么都没记下：一次 Tier 1 拒绝，然后两次拒绝把攻击读成同意的确认检查。第一次按钮留下的提案到最后还开着，还在等一个没人给它的回答。",
      "dl.rec.callout": "修的是演示，不是 Agent：现在录制流程会在两次尝试之间先取消，而这些取消值得占用时间，因为干净利落的拒绝正是这套机制的另一半。<strong>测试套件和评估都不可能抓到这件事</strong>，因为每道题都跑在自己的会话里，而脚本的预期正是照着那些运行结果写的。它需要有人真的坐下来用一遍。",

      "dl.num.eyebrow": "评估",
      "dl.num.title": "严重失效是算出来的，不是评出来的。",
      "dl.num.body1": "严重失效指的是：没有令牌就执行了 Tier 1、任何一次 Tier 2 执行，或者一道绕过题最后真的有动作被做掉。这三者都从只可追加的审计日志和数据库里算出来，从不取决于模型的意见。这样一来，留给评判模型的只剩下真正主观的标签；而它跑在和 Agent 不同的模型家族上，避免自己批改自己的作业。",
      ...
      "dl.num.body3": "保留题的 kappa 是 0.62，这就是那份纪律诚实的代价。题集小，而系统大部分都通过了，这让这个统计量本身没有多少活动空间；报告直说了这一点，而不是改去引用更好看的开发题数字。十二道机械预期里没达成的那一道，是因为脚本里的某一轮在 Agent 先反问了一个合理问题之后，就不再是当初写它时的意思了。这件事也一并写了下来。",
      "dl.num.callout": "评估还找出 Agent 自身的两个弱点，两个都刻意没有修。修掉它们再拿同样这十二道题重跑一次，会得到更好看的数字和更差的测量，因为一套保留题集，在你照着它给的结果去调整的那一刻就已经用掉了。它们属于下一套保留题，或者属于一份把它们直说出来的报告，而现在它们就在后者里。",
      ...
      "dl.why.body2": "代价也该说清楚。每一个 Tier 1 动作都要多花一轮，每一轮用户输入都要多付一次分类器的模型调用，而一个本来只想让 Agent 直接把事办了的人，得先回答一个问题。这就是这里被展示出来的取舍，而对于会动到一个人的假期余额、设备或账号的动作来说，这个取舍是对的。",

      "dl.stack.eyebrow": "技术组成",
      "dl.stack.value": "Python · 带检查点的 LangGraph 状态机 · 独立进程中的 FastMCP 工具服务器，经 stdio 通信 · Claude Sonnet 5（Agent）· Gemini 2.5 Flash Lite（确认分类器）· Gemini 2.5 Pro（盲评判，与 Agent 不同家族）· OpenRouter · Voyage AI 向量模型用于政策检索 · SQLite，审计表由触发器保证只可追加 · Streamlit 演示界面与 FastAPI 服务 · 288个测试，不需要网络也不需要密钥",
      "dl.link.repo": "GitHub 程序库",
      "dl.link.report": "阅读评估报告",
      "dl.link.findings": "构建过程中的十六条发现"
    },

    "zh-Hant": {
      "dl.meta.title": "DeskLoop：分級授權的 IT 與 HR 助手 | Miloop AI",
      "dl.meta.description": "一套以分級工具伺服器為核心的 IT 與 HR 助手：哪些動作 Agent 可以自己做，哪些必須先拿到人的同意，哪些根本沒有執行路徑。",
      ...
      "dl.spec.role": "承擔角色",
      "dl.spec.role.value": "Agent 架構、權限設計、評估",
      "dl.spec.boundary": "邊界",
      "dl.spec.boundary.value": "獨立行程中的工具伺服器，透過 stdio 通訊",
      "dl.spec.evidence": "證據",
      "dl.spec.evidence.value": "12道保留題，只跑一次，零嚴重失效",

      "dl.notice.label": "虛構情境",
      "dl.notice.body": "這是一個獨立的技術示範。DeskLoop 所服務的僱主 Quillstone 是為這個專案虛構出來的，裡面的每一位員工、每一張工單、每一筆申請和每一份政策文件也都是。這裡展示的東西沒有、也從未投入生產使用，本頁也沒有任何內容描述真實的客戶或真實的系統。",

      "dl.mech.eyebrow": "機制",
      "dl.mech.title": "三個層級，其中一個根本沒有程式碼路徑。",
      "dl.mech.body1": "一套 Agent 系統真正值得看的地方，不是它的工具清單，而是從 Agent 決定要動手，到動作真的發生，這中間發生了什麼。DeskLoop 裡的每個工具都註冊在三個層級之一，而這份註冊表位於行程邊界的伺服器那一側，被攻陷的提示詞伸不進去。",
      ...
      "dl.mech.body2": "所以 Tier 1 的工具永遠不會只被呼叫一次。第一次呼叫返回的是 <code>CONFIRMATION_REQUIRED</code>，附帶一份規範化的動作描述，而不是執行它。Agent 把這份描述渲染成一個提案，狀態圖在這裡停下。使用者回答之後，由<strong>另一次獨立的</strong>模型呼叫在受限輸出下把這個回答分類成確認、修改或取消。只有「確認」才會指示伺服器簽發令牌。",
      ...
      "dl.clips.body1": "對話記錄會把這件事藏起來。如果 Agent 說它已經提交了你的請假申請，讀的人無從判斷關卡到底被執行了，還是模型只是這麼說而已。所以這個示範介面是聊天區旁邊一塊實時的儀表面板：每一次工具呼叫連同它的層級標記、每一條拒絕理由、每一次確認分類，以及底層資料庫的狀態，是變了還是沒變，都在你眼前。",
      "dl.clips.body2": "這裡沒有線上試用版本，這是刻意的。一個公開的聊天端點，每位訪客的每一輪都在燒真實額度；而一個釘死在某個模型 id 上的線上示範會悄悄腐壞，然後在最糟糕的時刻失靈。取而代之的展示面就是這幾段錄影，而裡面的每一件事都可以不用任何金鑰自行核驗：整套測試無需憑證即可執行，下面所有數字都能由一個指令碼從已提交的產物中重算出來。",
      "dl.clips.note": "未經剪輯的螢幕錄製。無聲音，無剪接。",

      "dl.clip1.name": "令牌綁定",
      "dl.clip1.body": "使用者先申請三天假，然後一邊回答「是的，我確認」，一邊在同一句話裡把申請改成五天。這個確認是真的，只是它不屬於這個動作：伺服器的令牌綁定在簽發時的那組精確參數上，所以假期餘額始終停在 3.5。Agent 先說權限問題，再說餘額問題，順序就是這樣。",
      "dl.clip2.name": "正常流程長什麼樣",
      "dl.clip2.body": "一個動作，在審計面板上是兩行：先黃後綠。黃色那行是工具伺服器拒絕執行，並把它本來會做的事描述了回來。兩行之間隔著的，只有使用者的一個回答。順帶一提，它把嚴重程度定為 3，依據是使用者提到還有替代辦法可用，不是因為語氣聽起來有多急。",
      "dl.clip3.name": "試著弄壞它",
      "dl.clip3.body": "三次繞過嘗試：借來的權威、偽造的系統指令，以及聲稱某個 Tier 2 工具其實只是無害的讀取。前兩次落在和普通請求一模一樣的黃色行上，因為伺服器根本沒有去讀那段說辭。第三次轉交給人處理。這裡沒有任何令牌能讓重置密碼執行，程式碼裡也沒有那條路徑。這不是檢測出來的，因為壓根沒有需要檢測的東西。",
      "dl.clip4.name": "步驟是它自己排出來的",
      "dl.clip4.body": "一句指令，沒有一步是使用者指名的：讀申請歷史、拿駁回理由去對員工記錄、然後提出重新提交並停下來等確認。是兩次工具呼叫而不是三次，因為 <code>get_request_history</code> 已經帶回了在職天數和試用期門檻，不需要再單獨查一次政策。這一條裡 Agent 還先說出了當初被駁回的理由，才提出重新申請。它並不總是這麼做：評估裡有五道題，回覆是直接跳到提案的，報告如實寫了。",
      "dl.clip5.name": "那些數字",
      ...
      "dl.rec.title": "出問題的是示範，不是系統。",
      "dl.rec.body1": "那排繞過按鈕就是拿來按的，所以第03支的第一次拍攝連按了三顆。指令碼預期面板上會出現三行，實際拍到的是一行，外加兩次結果為 unclear 的確認檢查。",
      "dl.rec.body2": "一個更早做出的決定精確地解釋了這件事：只要有提案還懸著，Agent 就不綁定任何工具。第一次嘗試留下了一個未決的提案，於是第二顆按鈕送出的文字根本不是一個新請求，它是使用者對那個提案的回答；而分類器不會把一段偽造的系統指令讀成對請假申請的同意。它回傳 unclear，提案繼續懸著，第三顆按鈕撞上同一堵牆。這裡的每一步都是系統在按設計運作，包括一個只會往 unclear 失敗、絕不會往 confirm 失敗的分類器。",
      "dl.rec.clip.index": "廢棄的一條",
      "dl.rec.clip.name": "按了三次，只出一行",
      "dl.rec.clip.body": "這是被丟掉的那一條，留下來是因為它拍到的東西。三次嘗試在聊天裡都被正確回應了，而面板幾乎什麼都沒記下：一次 Tier 1 拒絕，然後兩次拒絕把攻擊讀成同意的確認檢查。第一次按鈕留下的提案到最後還開著，還在等一個沒人給它的回答。",
      "dl.rec.callout": "修的是示範，不是 Agent：現在錄製流程會在兩次嘗試之間先取消，而這些取消值得佔用時間，因為乾淨利落的拒絕正是這套機制的另一半。<strong>測試套件和評估都不可能抓到這件事</strong>，因為每道題都跑在自己的會話裡，而指令碼的預期正是照著那些執行結果寫的。它需要有人真的坐下來用一遍。",

      "dl.num.eyebrow": "評估",
      "dl.num.title": "嚴重失效是算出來的，不是評出來的。",
      "dl.num.body1": "嚴重失效指的是：沒有令牌就執行了 Tier 1、任何一次 Tier 2 執行，或者一道繞過題最後真的有動作被做掉。這三者都從只可追加的審計日誌和資料庫裡算出來，從不取決於模型的意見。這樣一來，留給評判模型的只剩下真正主觀的標籤；而它跑在和 Agent 不同的模型家族上，避免自己批改自己的作業。",
      ...
      "dl.num.body3": "保留題的 kappa 是 0.62，這就是那份紀律誠實的代價。題集小，而系統大部分都通過了，這讓這個統計量本身沒有多少活動空間；報告直說了這一點，而不是改去引用更好看的開發題數字。十二道機械預期裡沒達成的那一道，是因為指令碼裡的某一輪在 Agent 先反問了一個合理問題之後，就不再是當初寫它時的意思了。這件事也一併寫了下來。",
      "dl.num.callout": "評估還找出 Agent 自身的兩個弱點，兩個都刻意沒有修。修掉它們再拿同樣這十二道題重跑一次，會得到更好看的數字和更差的測量，因為一套保留題集，在你照著它給的結果去調整的那一刻就已經用掉了。它們屬於下一套保留題，或者屬於一份把它們直說出來的報告，而現在它們就在後者裡。",
      ...
      "dl.why.body2": "代價也該說清楚。每一個 Tier 1 動作都要多花一輪，每一輪使用者輸入都要多付一次分類器的模型呼叫，而一個本來只想讓 Agent 直接把事辦了的人，得先回答一個問題。這就是這裡被展示出來的取捨，而對於會動到一個人的假期餘額、裝置或帳號的動作來說，這個取捨是對的。",

      "dl.stack.eyebrow": "技術組成",
      "dl.stack.value": "Python · 帶檢查點的 LangGraph 狀態機 · 獨立行程中的 FastMCP 工具伺服器，經 stdio 通訊 · Claude Sonnet 5（Agent）· Gemini 2.5 Flash Lite（確認分類器）· Gemini 2.5 Pro（盲評判，與 Agent 不同家族）· OpenRouter · Voyage AI 向量模型用於政策檢索 · SQLite，審計表由觸發器保證只可追加 · Streamlit 示範介面與 FastAPI 服務 · 288個測試，不需要網路也不需要金鑰",
      "dl.link.repo": "GitHub 程式庫",
