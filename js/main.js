...
(function () {
  ...
  var dict = {
    "en": {
      ...
      "privacy.providers.body": "We use Resend to deliver the emails generated by the contact panel, Vercel to host this website, and Google's Gemini API to power the optional virtual assistant inside the contact panel. These providers process data on our behalf and are bound by their own privacy and security practices.",
      "privacy.assistant.title": "Virtual assistant",
      "privacy.assistant.body": "If you choose to use the virtual assistant inside the contact panel, your messages there are sent to Google's Gemini API to help identify which of our services fit your needs. Please don't share sensitive information, such as phone numbers or unannounced business plans, in that conversation. We keep a copy of these conversations to review the assistant's accuracy. We don't use them for advertising, and we don't share them outside Miloop AI.",
    ...
    "zh-Hans": {
      ...
      "privacy.providers.body": "我们使用Resend发送对话面板产生的邮件，使用Vercel托管本网站，并使用Google的Gemini API为对话面板里可选的虚拟助理提供支持。这些服务商代表我们处理数据，并各自遵循自己的隐私与安全规范。",
      "privacy.assistant.title": "虚拟助理",
      "privacy.assistant.body": "如果您选择使用对话面板里的虚拟助理，您在那段对话里发送的内容会传送给Google的Gemini API，用于判断哪些服务符合您的需求。请不要在那段对话里透露敏感信息，比如电话号码或尚未公开的商业计划。我们会保留这些对话记录，用于复查助理判断的准确性，不会用于广告投放，也不会分享给Miloop AI以外的任何人。",
    ...
    "zh-Hant": {
      ...
      "privacy.providers.body": "我們使用Resend發送對話面板產生的郵件，使用Vercel託管本網站，並使用Google的Gemini API為對話面板裡可選的虛擬助理提供支援。這些服務商代表我們處理資料，並各自遵循自己的隱私與安全規範。",
      "privacy.assistant.title": "虛擬助理",
      "privacy.assistant.body": "如果您選擇使用對話面板裡的虛擬助理，您在那段對話裡傳送的內容會傳送給Google的Gemini API，用於判斷哪些服務符合您的需求。請不要在那段對話裡透露敏感資訊，比如電話號碼或尚未公開的商業計畫。我們會保留這些對話紀錄，用於複查助理判斷的準確性，不會用於廣告投放，也不會分享給Miloop AI以外的任何人。",
